...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category','App']).agg({'Reviews' : 'sum', 'Rating' :'mean'}).sort_values(by=['Reviews'],ascending = False).iloc[0:10].reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    df3 = playstore.copy()
    top_category =  pd.crosstab(
    index=df3['Content Rating'],
    columns='Total',
    ).sort_values('Total', ascending=False)
    top_category=top_category.reset_index().head()
    ## Bar Plot Jumlah Content Rating
    cat_order = df3.groupby(['Content Rating']).agg({
